...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($array['image'])) {
        ...
        if (!$array['id']) {
            ...
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (title, description, link_href, link_target, image, addtime, edittime, weight, status) VALUES (
				:title, :description, :link_href, :link_target, :image, ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', ' . $weight . ', :status
			)';
        } else {
            $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET 
				title = :title, description = :description, link_href = :link_href, link_target = :link_target, image = :image, edittime = ' . NV_CURRENTTIME . ', status = :status 
				WHERE id = ' . $array['id'];
        }

        $array['description'] = nv_nl2br($array['description']);
