...
if (! empty($array_config['money_unit']) != '' and isset($currencies_array[$array_config['money_unit']])) {
    $page_title .= ' ' . $lang_module['money_compare'] . ' ' . $currencies_array[$array_config['money_unit']]['currency'];
}
...
if (! empty($savecat)) {
    ...
    if (isset($currencies_array[$data['code']])) {
        ...
        if (! empty($array_config['money_unit']) and $array_config['money_unit'] == $data['code']) {
            $data['exchange'] = 1;
        }
...
while ($row = $result->fetch()) {
    ...
    $row['link_del'] = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=money&id=' . $row['id'];
...
$xtpl->assign('URL_DEL', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=money');
