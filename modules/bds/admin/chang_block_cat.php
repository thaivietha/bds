...
if ($mod == 'weight' and $new_vid > 0) {
    $sql = 'SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_' . $module_data . '_block_cat WHERE bid=' . $bid;
    ...
    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_block_cat SET weight=' . $new_vid . ' WHERE bid=' . $bid;
    $db->query($sql);
    
    $content = 'OK_' . $bid;
} elseif ($mod == 'adddefault' and $bid > 0) {
    $new_vid = (intval($new_vid) == 1) ? 1 : 0;
    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_block_cat SET adddefault=' . $new_vid . ' WHERE bid=' . $bid;
    $db->query($sql);
    $content = 'OK_' . $bid;
} elseif ($mod == 'numlinks' and $new_vid >= 0 and $new_vid <= 50) {
    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_block_cat SET numbers=' . $new_vid . ' WHERE bid=' . $bid;
    $db->query($sql);
    $content = 'OK_' . $bid;
}
