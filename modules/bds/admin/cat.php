...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['css'] = $nv_Request->get_title('css', 'post', '');
    ...
    if (empty($error)) {
        ...
        try {
            if (empty($row['id'])) {
                $data_insert = array();
                $_sql = 'INSERT INTO ' . $table_name . ' (parentid, groups_view, image, css, weight' . $listfield . ') VALUES (:parentid, :groups_view, :image, :css, :weight' . $listvalue . ')';
                ...
                $stmt = $db->prepare('UPDATE ' . $table_name . ' SET parentid=:parentid, groups_view = :groups_view, css = :css, image = :image, ' . NV_LANG_DATA . '_title = :title, ' . NV_LANG_DATA . '_alias = :alias, ' . NV_LANG_DATA . '_custom_title = :custom_title, ' . NV_LANG_DATA . '_keywords = :keywords, ' . NV_LANG_DATA . '_description = :description, ' . NV_LANG_DATA . '_description_html = :description_html WHERE id=' . $row['id']);
                ...
                $stmt->bindParam(':alias', $row['alias'], PDO::PARAM_STR);
                $stmt->bindParam(':css', $row['css'], PDO::PARAM_STR);
