...
if (empty($contents)) {
    ...
    if ($viewcat == 'viewcat_page_new' or $viewcat == 'viewcat_page_old' or $set_view_page) {
        ...
        $db_slave->select('id, listcatid, admin_id, topicid, admin_id, author, sourceid, addtime, edittime, publtime, title, alias, hometext, homeimgfile, homeimgalt, homeimgthumb, allowed_rating, external_link, hitstotal, hitscm, total_rating, click_rating')
            ->order($order_by)
            ->limit($per_page)
            ->offset(($page - 1) * $per_page);
        ...
        while ($item = $result->fetch()) {
            ...
            $array_user = array();
            $sql = 'SELECT userid, first_name, last_name, photo FROM bcb_users WHERE userid = ' . $item['admin_id'];
            $user_list = $nv_Cache->db($sql, 'userid', 'users');
            if(!empty($user_list))
            {
            ...
            $item['photo'] =  '/' . $array_user['photo'];

            $item['admin'] = $array_user['last_name'] . ' ' . $array_user['first_name'];
