...
if (! nv_function_exists('nv_block_news_groups')) {
    ...
    {
        global $module_array_cat, $site_mods, $module_config, $global_config, $nv_Cache, $db, $lang_module, $global_array_cat;
        $module = $block_config['module'];
        $show_no_image = $module_config[$module]['show_no_image'];
        $blockwidth = $module_config[$module]['blockwidth'];

        $db->sqlreset()
            ->select('t1.id, t1.catid, t1.title, t1.alias, t1.homeimgfile, t1.homeimgthumb,t1.hometext,t1.publtime, t1.hitstotal, t1.external_link, t1.admin_id')
        ...
        if (! empty($list)) {
            ...
            foreach ($list as $l) {
                ...
                $array_user = array();
                $sql = 'SELECT userid, first_name, last_name, photo FROM bcb_users WHERE userid = ' . $l['admin_id'];
                $user_list = $nv_Cache->db($sql, 'userid', $module);
                if(!empty($user_list))
                {
                ...
                $l['photo'] =  $array_user['photo'];

                $l['admin'] = $array_user['last_name'] . ' ' . $array_user['first_name'];
